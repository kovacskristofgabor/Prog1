...
Lines_window::Lines_window(Point xy, int w, int h, const string& title)
    : Window{xy, w, h, title},
    next_button{Point{x_max() - 150, 0}, 70, 20, "Kovetkezo pont",
        [](Address, Address pw) { reference_to<Lines_window>(pw).next(); }},
    quit_button{Point{x_max() - 70, 0}, 70, 20, "Kilepes",
        [](Address, Address pw) { reference_to<Lines_window>(pw).quit(); }},
    next_x{Point{x_max() - 310, 0}, 50, 20, "kovetkezo x:"},
    next_y{Point{x_max() - 210, 0}, 50, 20, "kovetkezo y:"},
    xy_out{Point{100, 0}, 100, 20, "jelenlegi (x,y):"},
    color_menu{Point{x_max() - 70, 30}, 70, 20, Menu::vertical, "szin"},
    color_button{Point{x_max() - 80, 30}, 80, 20, "szin menu",
        [](Address, Address pw) {
            reference_to<Lines_window>(pw).color_pressed();
        }
    },
    style_menu{Point{10, 30}, 70, 20, Menu::vertical, "stilus"},
    style_button{Point{0, 30}, 80, 20, "stilus menu",
        [](Address, Address pw) {
    ...
    }
{
    ...
    attach(next_y);
    attach(xy_out);
    xy_out.put("nincs pont");
    color_menu.attach(new Button{Point{0, 0}, 0, 0, "piros",
        [](Address, Address pw) {
            reference_to<Lines_window>(pw).red_pressed();
        }
    });
    color_menu.attach(new Button{Point{0, 0}, 0, 0, "kek",
        [](Address, Address pw) {
    ...
    style_menu.attach(new Button{Point{0, 0}, 0, 0, "pont",
        [](Address, Address pw) {
    ...
    style_menu.attach(new Button{Point{0, 0}, 0, 0, "vonal",
        [](Address, Address pw) {
...
/* Compile command
g++ -w -Wall -std=c++11 Graph.cpp Window.cpp GUI.cpp Simple_window.cpp drill16.cpp `fltk-config --ldflags --use-images` -o drill16
*/
